...
struct fmt::formatter<Tag<N>> : fmt::formatter<int> {
    auto format(Tag<N> t, fmt::format_context& ctx) const { return fmt::formatter<int>::format(t.v, ctx); }
};
...
#define R10(M, N) M(N + 0) M(N + 1) M(N + 2) M(N + 3) M(N + 4) M(N + 5) M(N + 6) M(N + 7) M(N + 8) M(N + 9)
#define R50(M, N) R10(M, (N) + 0) R10(M, (N) + 10) R10(M, (N) + 20) R10(M, (N) + 30) R10(M, (N) + 40)
