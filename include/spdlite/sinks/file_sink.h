...
struct file_sink {
    ...
    void write(const log_msg& msg) { detail::fwrite_bytes(msg.formatted.data(), msg.formatted.size(), file_.get()); }
...
inline file_sink::file_sink(const std::filesystem::path& filename, open_mode mode) {
